...
try:
    time_str = os.environ["TIME_START"].strip("'\"")
    time_obj = datetime.strptime(time_str, "%Y-%m-%d %H:%M")
    logger.info(f" :: TIME_START = {time_obj}")
except (KeyError, ValueError):
    logger.info(" ---> Get TIME_START ... FAILED")
    logger.error(" ===> TIME_START must be set and must be a valid datetime in YYYY-MM-DD HH:MM format")
    sys.exit(1)

time_start = time_obj
logger.info(" ---> Get TIME_START ... DONE")

logger.info(" ---> Get TIME_END ... ")
try:
    time_str = os.environ["TIME_END"].strip("'\"")
    time_obj = datetime.strptime(time_str, "%Y-%m-%d %H:%M")
    logger.info(f" :: TIME_END = {time_obj}")
except (KeyError, ValueError):
    logger.info(" ---> Get TIME_END ... FAILED")
    logger.error(" ===> TIME_END must be set and must be a valid datetime in YYYY-MM-DD HH:MM format")
    sys.exit(1)
    
time_end = time_obj
logger.info(" ---> Get TIME_END ... DONE")
