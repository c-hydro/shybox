...
# ----------------------------------------------------------------------------------------------------------------------
# method to compute evapotranspiration using domain mask, gaps filling and weighted mean
...
def compute_et(
        ...
        var_name_in: str = 'evapotranspiration', var_name_out: str = 'evapotranspiration',
        ...
        thr_et_min_flag: bool = False, thr_et_max_flag: bool = False,
        thr_et_min_value: float = 0.0, thr_et_max_value: float = 1.0,
        debug_box: bool = False, debug_geo: bool = False, debug_data_raw: bool = False,
        debug_data_by_watermark: bool = False,
    ...
    logger_stream.info_up("Compute evapotranspiration ... ")
    ...
    for ds_id, (ds_vars, da_wm) in enumerate(zip(ds_list, wm_list)):
        ...
        # apply evapotranspiration thresholds (keep only where et is between min and max, set to NaN otherwise)
        if thr_et_min_flag:
            values_tmp = np.where(values_tmp < thr_et_min_value, np.nan, values_tmp)
        if thr_et_max_flag:
            values_tmp = np.where(values_tmp > thr_et_max_value, np.nan, values_tmp)

        # check results
        if debug_geo:
            plot_data(values_wm, title=f"et - watermark", plot_block=True)
        if debug_data_raw:
            plot_data(da_in.values, title=f"et - data raw", plot_block=True)
        if debug_data_by_watermark:
            plot_data(values_tmp, title=f"et - data filtered by wm", plot_block=True)
        ...
        if debug_data_by_resampling:
            plot_data(da_tmp.values, title=f"et - data resample/interp method", plot_block=True)
        if debug_data_merge_domain_partial:
            plot_data(merge_out_raw, title=f"et - merge domain partial", plot_block=True)
    ...
    if fill_gaps_flag:
        ...
        if debug_data_merge_domain_gaps:
            plot_data(merge_out_gaps, title=f"et - merge domain gaps", plot_block=True)
            plot_data(merge_out_raw, title=f"et - merge domain out - before filling gaps", plot_block=True)
        ...
        # check results
        if debug_data_merge_domain_gaps:
            plot_data(merge_out_def, title=f"et - merge domain out - after filling gaps", plot_block=True)
    ...
    if debug_data_merge_domain_merged:
        plot_data(merge_out_def, title=f"et - merge domain out", plot_block=True)
    ...
    if debug_data_merge_domain_weighted:
        plot_data(merge_out_wg, title=f"et - weighted domain out", plot_block=True)
    ...
    # algorithm end
    logger_stream.info_down("Compute evapotranspiration ... DONE")
